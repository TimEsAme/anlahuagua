    ...
    <table class="table-container">
      ...
      <tbody>
        <tr v-for="item in list" :key="item.id">
          ...
          <td>{{ item.phone }}</td>
          <td>
            <el-button link type="primary" size="small" @click="goToDetail(item)">详情</el-button>
          </td>
        </tr>
      </tbody>
    </table>
...
// 获取学生列表数据
// onMounted(async () => {
//   await store.fetchUser() // 调用 Pinia store 的 fetchUser 方法获取数据
// })

// 传入一个回调函数，该回调函数会在服务器端执行
useData(async () => {
